...
function connectDomainSocket_p(sockName, socketPath, interval, timeout, shouldContinue) {
  ...
  var intervalId = setInterval(function() {
    ...
    // chmod the socket file permission to 0600, since they are per default
    // created with no permission bits set
    try {
      fs.chmodSync(socketPath, 0600)
    } catch (err) {
      logger.warn("Could not change permissions on the socket file. Already closed?")
    }
